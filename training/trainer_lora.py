...
class ChameleonTrainer(Trainer):
    ...
    def vl_training_step(self, model, inputs):
        ...
        inputs = self.processor(text, vqmodel=model.module.model.model.vqmodel, images=images, return_tensors="pt", padding='max_length', max_length=1300, truncation=True)
        ...
        # model forward
        logits, recon_image_latents = model(
            input_ids = input_ids,
        ...
        loss_mse = F.mse_loss(recon_image_latents, image_latents.clone().detach())
        # print(f'mse loss: {loss_mse}')

        loss = 1*loss_ce + 0*loss_mse
