...
class GenerationMixin:
    ...
    def generate(
        self,
        generate_wo_quant: Optional[bool] = False,
        inputs: Optional[torch.Tensor] = None,
        ...
        **kwargs,
        ...
        if generation_mode == GenerationMode.ASSISTED_GENERATION:
            ...
            if generate_wo_quant == True:
                # 13. run sample (it degenerates to greedy search when `generation_config.do_sample=False`)
                result = self._sample_wo_quant(
                    input_ids,
                ...
                # 13. original version of run sample (it degenerates to greedy search when `generation_config.do_sample=False`)
                result = self._sample(
                    input_ids,
                    image_latents=image_latents,
                    logits_processor=prepared_logits_processor,
    ...
    ### added sample method for chameleon_wo_quant
    def _sample_wo_quant(
        self,
        input_ids: torch.LongTensor,
        ...
        **model_kwargs,
        ...
        if image_latents == None: #there is no image in the prompt
            ...
            collect_image_latents = [image_latent.squeeze(1).squeeze(0) for image_latent in torch.split(image_latents, 1, dim=1)]
            # print(len(collect_image_latents)) [1024]
            # print(collect_image_latents[0].shape) torch.Size([256])
        generate_eoi = False
        ...
        while self._has_unfinished_sequences(
            ...
            if len(collect_image_latents) == 0:
                outputs = self(**model_inputs, return_dict=True, mode=mode, logits_processor=logits_processor, logits_warper=logits_warper)
            else:
                image_latents = torch.stack(collect_image_latents, dim=0)
                outputs = self(**model_inputs, image_latents=image_latents.unsqueeze(dim=0), return_dict=True, mode=mode, \
            ...
            if mode == 'discrete':
                ...
                # we use <image>(token_id: 8711) to represent image_token
                ...
                if sum_image_latents_generated%1024 ==0:
                    generate_eoi = True
                    mode = 'discrete'
                    return torch.stack(collect_image_latents, dim=0)
            else: 
                raise ValueError(
