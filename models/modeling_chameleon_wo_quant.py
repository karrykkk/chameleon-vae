...
from .modeling_outputs_wo_quant import (
...
def top_p_sampling(logits, p=0.9):
    ...
    cum_probs = torch.cumsum(F.softmax(sorted_logits/0.7, dim=-1), dim=-1)

    # Find the cutoff index such that the cumulative probability is >= p
    cutoff = (cum_probs >= p).nonzero(as_tuple=True)[0][0] + 1

    # Mask out the logits below the cutoff
    ...
    sampled_token = top_p_indices.gather(1, torch.multinomial(probs, num_samples=1))
...
def _prepare_4d_causal_attention_mask_with_cache_position(
    ...
    if attention_mask is not None and attention_mask.dim() == 4:
        ...
        if sequence_length != 1:
            causal_mask = torch.triu(causal_mask.float(), diagonal=1)
...
class ChameleonVQVAEDecoderConvUpsample(nn.Module):
    def __init__(self, in_channels):
        ...
        self.upsample = UpsampleDeterministic(upscale=2)
...
class ChameleonVQVAE(PreTrainedModel):
    ...
    def encode_wo_quant(self, pixel_values: torch.FloatTensor) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.LongTensor]:
        hidden_states = self.encoder(pixel_values)
        hidden_states = self.quant_conv(hidden_states)
        return hidden_states       
...
class ChameleonModel(ChameleonPreTrainedModel):
    ...
    def __init__(self, config: ChameleonConfig):
        ...
        self.T = 1 # temperature for softmax
    ...
    def forward(
        ...
        image_latents: torch.LongTensor = None,
        attention_mask: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
        ...
        if image_latents is not None and inputs_embeds is not None:
            raise ValueError(
        ...
        if image_latents is None and inputs_embeds is None: # text-only input
            ...
            # print(f'3: {self.codebook_in.weight}')
            ...
            # print(f'4: {self.vqmodel.quantize.embedding.weight}')
            for id in range(bsz):
                image_latents_ = image_latents[id].view(-1, self.vqmodel.quantize.embedding_dim)
                # distances from z to embeddings e_j (z - e)^2 = z^2 + e^2 - 2 e * z
                distances = (
                    torch.sum(image_latents_**2, dim=1, keepdim=True)
                    + torch.sum(self.codebook_in.weight**2, dim=1)
                    - 2 * torch.einsum("bd,dn->bn", image_latents_, self.codebook_in.weight.transpose(0, 1))
                )
                weighted_indices = F.softmax(-distances / self.T, dim=-1)
                # print(torch.nonzero(weighted_indices).shape[0])
                image_embeds = torch.matmul(weighted_indices.to(self.embed_tokens.weight.dtype), self.embed_tokens.weight[4:8196])
                image_embeds = image_embeds.view(bsz, -1, self.config.hidden_size)
                # print(inputs_embeds[id, image_mask[id]==1].shape)
...
class ChameleonForConditionalGeneration(ChameleonPreTrainedModel):
    ...
    def __init__(self, config: ChameleonConfig):
        ...
        # add new traning parameter for C_out & initialize as pre-trained codebook
        self.codebook_out = nn.Embedding(self.model.vqmodel.quantize.num_embeddings, self.model.vqmodel.quantize.embedding_dim)
    ...
    def forward(
        ...
        if self.model.training:
            #TODO:support mutiple images output
            # get the position of image_tokens (<boi>, <eoi> are excluded.)
            image_mask = input_ids==8711
            bsz = image_mask.shape[0]
            recon_image_latents = []
            # print(f'5: {self.model.vqmodel.quantize.embedding.weight}')
            # print(f'6: {self.codebook_out.weight}')
            for id in range(bsz):
                boi_position = torch.nonzero(image_mask[id])[0] - 1
                image_logits = logits[id, boi_position : boi_position + 1024, :]
                weighted_indices = F.softmax(image_logits[:, 4:8196] / self.model.T, dim=-1)
                # weighted_indices = F.gumbel_softmax(image_logits[:, 4:8196], hard=False, tau=1, dim=-1)
                recon_image_hidden_states = torch.matmul(weighted_indices, self.codebook_out.weight)
                recon_image_hidden_states = recon_image_hidden_states.view((*self.model.vqmodel.quantize.quant_state_dims, self.codebook_out.weight.shape[-1]))
                recon_image_latents.append(recon_image_hidden_states.contiguous())
            recon_image_latents = torch.stack(recon_image_latents, dim=0)
            return logits, recon_image_latents

        elif mode == 'discrete': # discrete output
            logits = logits.float()
            next_image_latents = None
        elif mode == 'continuous': # continuous output
            next_image_token_logits = logits[:, -1, :]

            #### original generation ####
            # pre-process distribution
            next_image_token_scores = logits_processor(input_ids, next_image_token_logits)
            next_image_token_scores = logits_warper(input_ids, next_image_token_scores)
            #### original 'do_sample=True' generation
            # probs = nn.functional.softmax(next_image_token_scores, dim=-1)
            # sampled_token = torch.multinomial(probs, num_samples=1).squeeze(1)
            # #### original 'do_sample=False' generation
            # sampled_token = torch.argmax(next_image_token_scores, dim=-1)
            # -4 is to change chameleon_image_id to vqgan_image_id
            # next_image_latents = self.codebook_out.weight[sampled_token[0]-4]

            #### quant-free generation ####
            # print(f'1: {self.model.vqmodel.quantize.embedding.weight}')
            # print(f'2: {self.codebook_out.weight}')
            # weighted_indices = F.softmax(next_image_token_logits[:, 4:8196] / self.model.T, dim=-1)
            # weighted_indices = F.gumbel_softmax(next_image_token_logits[:, 4:8196], hard=False, tau=0.0001, dim=-1)
            # next_image_latents = torch.matmul(next_image_token_scores, self.codebook_out.weight)
            weighted_indices = F.gumbel_softmax(next_image_token_scores, hard=False, tau=1, dim=-1)
            next_image_latents = torch.matmul(weighted_indices[:, 4:8196], self.codebook_out.weight)
        else:
            raise ValueError(
                "Unknown multimodal generation mode."
            )
